...
async function create({ card, token }) {
    ...
    const validate = await cardsRepo.findByTitleAndUserId(card.card_title, user.userId);
...
async function list({ token }) {
    ...
    const user = await sessionsService.getUserByToken(token);
    const cards = await cardsRepo.findByUserId(user.userId);
...
async function get(token: string, id: number) {
    ...
    const card = await cardsRepo.findUserCardById(user.userId, id);
